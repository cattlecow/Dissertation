...
from .network_blocks import DoubleConvBlock, AttentionBlock2d, DoubleConvBlock_dy
...
class UNet(nn.Module):
    ...
    def __init__(self, num_classes, in_channels, is_esa=False, is_grid=False, is_dy=False):
        super().__init__()
        ...
        if self.is_dy:
            # 下采样的模块
            self.conv0_0 = DoubleConvBlock_dy(in_channels, nb_filters[0], nb_filters[0])
            self.conv1_0 = DoubleConvBlock_dy(nb_filters[0], nb_filters[1], nb_filters[1])
            self.conv2_0 = DoubleConvBlock_dy(nb_filters[1], nb_filters[2], nb_filters[2])
            self.conv3_0 = DoubleConvBlock_dy(nb_filters[2], nb_filters[3], nb_filters[3])
            self.conv4_0 = DoubleConvBlock_dy(nb_filters[3], nb_filters[4], nb_filters[4])

            # 上采样的模块
            self.conv3_1 = DoubleConvBlock_dy(nb_filters[4] + nb_filters[3], nb_filters[3], nb_filters[3])
            self.conv2_2 = DoubleConvBlock_dy(nb_filters[3] + nb_filters[2], nb_filters[2], nb_filters[2])
            self.conv1_3 = DoubleConvBlock_dy(nb_filters[2] + nb_filters[1], nb_filters[1], nb_filters[1])
            self.conv0_4 = DoubleConvBlock_dy(nb_filters[1] + nb_filters[0], nb_filters[0], nb_filters[0])
        else:
            # 下采样的模块
