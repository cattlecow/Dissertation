...
'''Reference: https://github.com/ozan-oktay/Attention-Gated-Networks/blob/master/models/layers/grid_attention_layer.py'''
...
def conv3x3_dy(in_planes, out_planes, stride=1, groups=1, dilation=1):
...
class SingleConvBlock_dy(nn.Module):
    """Conv-BN-ReLU的基本模块，其中的Conv并不会张量的尺寸"""

    def __init__(self, in_channels, out_channels):
        super().__init__()
        self.block = nn.Sequential(
            conv3x3_dy(in_channels, out_channels, stride=1),
            nn.BatchNorm2d(out_channels),
            nn.ReLU(inplace=True)
        )
...
class DoubleConvBlock_dy(nn.Module):
    ...
    def __init__(self, in_channels, middle_channels, out_channels, dropout=False, p=0.2):
        ...
        # 将Dropout加在2个卷积层的中间
        layers = [SingleConvBlock_dy(in_channels, middle_channels)]
        if dropout:
            layers.append(nn.Dropout(p=p))
        layers.append(SingleConvBlock_dy(middle_channels, out_channels))
...
if __name__ == '__main__':
    ...
    img = torch.rand(2, 256, 16, 16)
    gate = torch.rand(2, 256, 16, 16)
    net = AttentionBlock2d(x_channels=256, gate_channels=256)
    out = net(img, gate)
    print(out.shape)